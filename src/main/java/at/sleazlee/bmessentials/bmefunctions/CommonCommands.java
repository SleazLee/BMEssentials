	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (sender instanceof Player) {
			...
			if (label.equalsIgnoreCase("lag")) {
				...
				String messageText = lagTitleStart + "<newline><hover:show_text:'<gold><bold>Memory:</bold><newline>Memory refers to RAM (Random Access Memory), which is used by the server to store and quickly access data. The more RAM a server has, the more information it can handle simultaneously, leading to smoother gameplay.'><gray>Memory: </hover><dark_gray>[<white>" + ramBar + "</white>] <hover:show_text:'<red><bold>Used Ram:</bold><newline>The amount of RAM currently being utilized by the server.'><yellow>" + ramUsed + " GB <gray>Used</hover>, <hover:show_text:'<green><bold>Free Ram:</bold><newline>The amount of RAM available and not currently in use by the server.'><green>" + ramUnused + " GB<gray> Free</hover><newline><hover:show_text:'<gray>TPS (Ticks Per Second) is a measure of the server's performance. In Minecraft, the game operates on a system of \"ticks,\" with the ideal rate being 20 TPS, meaning the server is processing 20 game cycles every second.<newline><newline><green>20 TPS<gray>: Indicates optimal server performance with no noticeable lag.<newline><yellow>18 TPS and above<gray>: Minor lag may occur but is generally unnoticeable.<newline><red>Below 18 TPS<gray>: Players may start experiencing server-side lag, affecting gameplay smoothness.'><hover:show_text:'<gray>TPS averaged over the last 1 minute.'><dark_gray>[<gray>1m: " + tpsOneMin + "]</hover> <hover:show_text:'<gray>TPS averaged over the last 5 minutes.'>[<gray>5m: " + tpsFiveMins + "]</hover> <hover:show_text:'<gray>TPS averaged over the last 15 minutes.'>[<gray>15m: " + tpsFifteenMins + "]</hover><hover:show_text:'<yellow><bold>Ping</bold><gray> measures the latency between your computer and the server, expressed in <aqua>milliseconds (ms)<gray>. It indicates how quickly your actions are communicated to the server and vice versa.<newline><newline><green>Lower Ping<gray>: Results in faster response times and smoother gameplay.<newline><red>Higher Ping<gray>: Can lead to delays, making actions feel laggy or unresponsive.'><gray> Your ping: <dark_gray>[<green>" + ping + " ms</hover>]<newline>" + lagTitleEnd;
